...
(function (__dirname, module) {
    ...
    function getRequestHandler(defaults, secure, app, cb) {
        return function (req, res, next) {
            ...
            return locate(defaults["public"], "js/async-backbone.js", function (err, manifestFile) {
                if (err) {
                    return cb(err);
                }
                ...
                return fs.readFile(manifestFile, "utf-8", function (err, content) {
                    ...
                    var body = template({
                        ...
                        ccontrol = app.logger.isDebugging ? 'NO-CACHE' : 'public, max-age=' + (options.maxAge / 1000),
                        headers = {
